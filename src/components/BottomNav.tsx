        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center py-1 px-1 transition-all duration-150 relative ${
                isActive ? 'text-ios-blue' : 'text-zinc-400 hover:text-zinc-700'
              }`}
            >
              <div className="relative">
                <Icon className={`w-5 h-5 transition-transform ${isActive ? 'scale-110' : ''}`} />
                {tab.badge && tab.badge > 0 ? (
                  <span className="absolute -top-1.5 -right-2.5 min-w-[16px] h-4 px-1 flex items-center justify-center text-[10px] font-bold text-white bg-rose-500 rounded-full border border-white animate-pulse">
                    {tab.badge}
                  </span>
                ) : null}
              </div>

              <span className={`text-[11px] mt-1 tracking-tight ${isActive ? 'font-bold text-ios-blue' : 'font-medium'}`}>
                {tab.label}
              </span>
